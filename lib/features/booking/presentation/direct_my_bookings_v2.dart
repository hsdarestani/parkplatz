...
class _DirectMyBookingsScreenState
  ...
  Widget build(BuildContext context) => FreiraumScaffold(
        ...
        subtitle: 'Anfragen, Parking Pass und Bewertungen verwalten.',
        activePath: '/bookings',
        actions: [
          FilledButton.icon(
                  ...
                  if (values.isEmpty)
                    ...
                    if (pending.isNotEmpty) ...[
                      _section(
                        ...
                        'Auch kostenlose und sofort reservierte Anfragen werden bestätigt.',
                      ),
                    ...
                    if (history.isNotEmpty) ...[
                      _section(
                        'Verlauf',
                        'Vergangene Aufenthalte können bewertet werden.',
                      ),
                      const SizedBox(height: 12),
                      ...history.map(_card),
                    ],
            ...
            const SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Anbieterbestätigung schützt Adresse, Zugang und Parking Pass.',
  ...
  Widget _card(
    ...
    final completed = booking.status == 'completed' ||
        (confirmed && booking.end.isBefore(DateTime.now()));
    ...
    final background = confirmed
        ...
        : pending
            ? T.amberSoft
            : T.porcelainDeep;
    final cancellingNow = cancelling.contains(booking.id);
    final reviewingNow = reviewing.contains(booking.id);
    return Container(
      ...
      child: Wrap(
        ...
        children: [
                ...
                Text(
                  pending
                      ? booking.totalCents == 0
                          ? 'Kostenlose Anfrage – Anbieterbestätigung ausstehend'
                          : 'Zahlung eingereicht – Anbieterbestätigung ausstehend'
                      : _statusLabel(booking.status),
                  style: TextStyle(color: color, fontWeight: FontWeight.w800),
                ),
                if (cancelled && booking.totalCents > 0)
                  const Text(
                    'Eine direkte Zahlung muss der Anbieter gegebenenfalls manuell erstatten.',
          ...
          Text(
            booking.totalCents == 0 ? 'Kostenlos' : _money(booking.totalCents),
            style: TextStyle(
              fontSize: 19,
              fontWeight: FontWeight.w900,
              color: booking.totalCents == 0 ? T.success : T.ink,
            ),
          ),
          ...
          if (completed)
            FilledButton.tonalIcon(
              onPressed: reviewingNow ? null : () => _review(booking),
              icon: reviewingNow
                  ? const SizedBox(
                      width: 16,
                      height: 16,
          ...
          if (cancellable)
            TextButton.icon(
              onPressed: cancellingNow ? null : () => _cancel(booking),
              icon: cancellingNow
                  ? const SizedBox(
                      width: 16,
                      height: 16,
  ...
  Future<void> _review(BookingRecord booking) async {
    var rating = 5;
    ...
    final result = await showDialog<(int, String)>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: const Text('Stellplatz bewerten'),
          content: Column(
          ...
          actions: [
            ...
            FilledButton.icon(
              onPressed: () {
                ...
                if (text.length >= 3) {
                  Navigator.pop(dialogContext, (rating, text));
                }
              },
    ...
    setState(() => reviewing.add(booking.id));
    try {
      await ref
          .read(marketplaceRepositoryProvider)
          .createReview(booking.id, result.$1, result.$2);
      ref.invalidate(parkingSpaceProvider(booking.parkingId));
      ref.invalidate(parkingReviewsProvider(booking.parkingId));
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Bewertung wurde veröffentlicht.')),
        );
      }
    } catch (error) {
      if (mounted) {
      ...
      if (mounted) setState(() => reviewing.remove(booking.id));
    }
  }
  ...
  Future<void> _cancel(BookingRecord booking) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          booking.status == 'confirmed' && booking.totalCents > 0
